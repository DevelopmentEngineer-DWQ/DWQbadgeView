...
#error DWQBadgeView must be compiled with ARC.
#endif
...
#define DWQBadgeViewSilenceDeprecatedMethodStart()   _Pragma("clang diagnostic push") \
_Pragma("clang diagnostic ignored \"-Wdeprecated-declarations\"")
#define DWQBadgeViewSilenceDeprecatedMethodEnd()     _Pragma("clang diagnostic pop")
#else
#define DWQBadgeViewSilenceDeprecatedMethodStart()
#define DWQBadgeViewSilenceDeprecatedMethodEnd()
#endif

static const CGFloat DWQBadgeViewShadowRadius = 1.0f;
static const CGFloat DWQBadgeViewHeight = 16.0f;
static const CGFloat DWQBadgeViewTextSideMargin = 8.0f;
static const CGFloat DWQBadgeViewCornerRadius = 10.0f;

// Thanks to Peter Steinberger: https://gist.github.com/steipete/6526860
static BOOL DWQBadgeViewIsUIKitFlatMode(void)
{
...
@implementation DWQBadgeView

+ (void)applyCommonStyle
{
    DWQBadgeView *badgeViewAppearanceProxy = DWQBadgeView.appearance;
    
    badgeViewAppearanceProxy.backgroundColor = UIColor.clearColor;
    badgeViewAppearanceProxy.badgeAlignment = DWQBadgeViewAlignmentTopRight;
...
+ (void)initialize
{
    if (self == DWQBadgeView.class)
    {
        [self applyCommonStyle];
        
        if (DWQBadgeViewIsUIKitFlatMode())
        {
            [self applyIOS7Style];
        }
        else
...
- (id)initWithParentView:(UIView *)parentView alignment:(DWQBadgeViewAlignment)alignment
{
...
- (void)layoutSubviews
{
    ...
    const CGFloat viewWidth = MAX(_badgeMinWidth, textWidth + DWQBadgeViewTextSideMargin + (marginToDrawInside * 2));
    const CGFloat viewHeight = DWQBadgeViewHeight + (marginToDrawInside * 2);
    ...
    switch (self.badgeAlignment) {
        case DWQBadgeViewAlignmentTopLeft:
            newFrame.origin.x = -viewWidth / 2.0f;
            newFrame.origin.y = -viewHeight / 2.0f;
            break;
        case DWQBadgeViewAlignmentTopRight:
            newFrame.origin.x = superviewWidth - (viewWidth / 2.0f);
            newFrame.origin.y = -viewHeight / 2.0f;
            break;
        case DWQBadgeViewAlignmentTopCenter:
            newFrame.origin.x = (superviewWidth - viewWidth) / 2.0f;
            newFrame.origin.y = -viewHeight / 2.0f;
            break;
        case DWQBadgeViewAlignmentCenterLeft:
            newFrame.origin.x = -viewWidth / 2.0f;
            newFrame.origin.y = (superviewHeight - viewHeight) / 2.0f;
            break;
        case DWQBadgeViewAlignmentCenterRight:
            newFrame.origin.x = superviewWidth - (viewWidth / 2.0f);
            newFrame.origin.y = (superviewHeight - viewHeight) / 2.0f;
            break;
        case DWQBadgeViewAlignmentBottomLeft:
            newFrame.origin.x = -viewWidth / 2.0f;
            newFrame.origin.y = superviewHeight - (viewHeight / 2.0f);
            break;
        case DWQBadgeViewAlignmentBottomRight:
            newFrame.origin.x = superviewWidth - (viewWidth / 2.0f);
            newFrame.origin.y = superviewHeight - (viewHeight / 2.0f);
            break;
        case DWQBadgeViewAlignmentBottomCenter:
            newFrame.origin.x = (superviewWidth - viewWidth) / 2.0f;
            newFrame.origin.y = superviewHeight - (viewHeight / 2.0f);
            break;
        case DWQBadgeViewAlignmentCenter:
            newFrame.origin.x = (superviewWidth - viewWidth) / 2.0f;
            newFrame.origin.y = (superviewHeight - viewHeight) / 2.0f;
            break;
        default:
...
{
    DWQBadgeViewSilenceDeprecatedMethodStart();
    return [self.badgeText sizeWithFont:self.badgeTextFont];
    DWQBadgeViewSilenceDeprecatedMethodEnd();
}

#pragma mark - Setters

- (void)setBadgeAlignment:(DWQBadgeViewAlignment)badgeAlignment
{
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (anyTextToDraw)
    {
        ...
        UIBezierPath *borderPath = [UIBezierPath bezierPathWithRoundedRect:rectToDraw byRoundingCorners:(UIRectCorner)UIRectCornerAllCorners cornerRadii:CGSizeMake(DWQBadgeViewCornerRadius, DWQBadgeViewCornerRadius)];
        ...
        CGContextSaveGState(ctx);
        {
            ...
            CGContextSetShadowWithColor(ctx, self.badgeShadowSize, DWQBadgeViewShadowRadius, self.badgeShadowColor.CGColor);
